...
exports.createNewPlayer = async(req,res)=>{
    ...
    await fs.writeFile(`${__dirname}/../data/players.json`,JSON.stringify(players,null,2),(err)=>{
        if(err){
            res.status(400).json({status:'fail',data:{status:'fail',data:{message:'failed to add player'}}});
        }
        res.status(200).json({status:'success',data:{player:newPlayer}});
    });
}
exports.updatePlayer = async (req,res)=>{
    ...
    await fs.writeFile(`${__dirname}/../data/players.json`,JSON.stringify(players,null,2),(err)=>{
        ...
        res.status(200).json({status:'success',data:{player:players[indexToUpdate]}});
    });
}
exports.deletePlayer = async (req,res)=>{
    ...
    await fs.writeFile(`${__dirname}/../data/players.json`,JSON.stringify(players,null,2),(err)=>{
        ...
        res.status(200).json({status:'success',data:{player:playerToRemove}});
    });
}
